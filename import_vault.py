...
.tm-inner{margin:0 auto;display:flex;flex-wrap:wrap;align-items:stretch;
  justify-content:center;gap:.25rem;padding:0 1rem}
.tm-item{position:relative;display:flex}
.tm-link{display:flex;align-items:center;color:#fff;opacity:.92;text-decoration:none;
  font-size:.78rem;font-weight:700;line-height:1;padding:.85rem 1.15rem;white-space:nowrap;
  cursor:pointer;border-radius:.2rem;transition:background .15s}
.tm-link:hover{opacity:1;background:rgba(255,255,255,.12)}
...
@media(max-width:76.1875em){
  .tm-inner{flex-wrap:nowrap;overflow-x:auto;justify-content:flex-start}
